...
def visualize_2(graph, show = True, save = False, epoch = False):
    nx_graph = graph.to_networkx().to_undirected()
    nx_graph.remove_nodes_from(remove)
    graph.ndata['x'][remove] = -1
    color = graph.ndata['x'][graph.ndata['x'] != -1]
    plt.figure(figsize=[20,11])
    ec = nx.draw_networkx_edges(nx_graph, pos=positions, alpha=0.7)
    nc = nx.draw_networkx_nodes(nx_graph, pos=positions, node_size=5, node_color=color, vmin=10, vmax=25, cmap=plt.cm.jet)
    ticks = np.linspace(graph.ndata['x'].min(), graph.ndata['x'].max(), 5, endpoint=True)
    plt.colorbar(nc, ticks=ticks)
...
def build_graph_from_csv(file):
    ...
    bg.ndata['x'] = torch.zeros(bg.num_nodes(), 1)

    for i, node in node_features_pd.iterrows():
        bg.ndata['x'][node] = node['features']
    return bg, edges
...
def MS_1layer(graph, mask, feature_name = 'x', max_iters = 1,  early_stop = 1e-3, history = False, original_feat = False, show = False, save = False):
